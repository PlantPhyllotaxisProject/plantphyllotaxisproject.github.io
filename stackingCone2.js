...
class StackingCone {
  ...
  */
  constructor(p, vertexX, vertexY, angle, diskRadius, height = 0.9, rMeridian, options) {
    ...
    this.windowHeight = p.height
    this.windowWidth = p.width/2
  ...
  maxRadius() {
    let testConeLattice = new ConeLattice(this.upPara,this.downPara,this.angle, this.minRadius()+0.001,0,this.p, this.diskRadius);
    ...
    let J = -c*(n-1)/(n) - c*(m-1)/(m) - c;
    let J2 = -c*(u-1)/(n) - c*(v-1)/m
    
    let j = p.sin(c/2)/p.sin(c/(2*n))
    let k = p.sin(c/2)/p.sin(c/(2*m))
    let K = p.PI-(1*p.PI/3) + c*(u-1)/(n) + c*(v-1)/(m) - c*(n-1)/(2*n) - c*(m-1)/(2*m) 
    let max_r = 0.5*p.sqrt(1/(j**2+k**2+2*k*j*p.cos(K)))
    this.p.print(max_r)
    return max_r
  }
...
Returns whichever is smallest. Note that returning negative numbers implies overlap; -1 would mean complete overlap, -0.5 would be half overlap, etc. Also note that this method is really only intended to be used in generateCustomFront and related methods as a way to determine whether more disks should be added to the front.
  ...
  display() {
    ...
    // for (let disk of this.frontDisks) {
    //   disk.displayDisk(50,[0,0,0,0])
    // }
  ...
  displayCylinder() {
    ...
    if (this.drawingMode == "log"){
      this.p.line(0.5, 0, 0.5, 1-this.vertexY);
      this.p.line(-0.5, 0, -0.5, 1-this.vertexY);
    }
    else {
      this.p.line(-this.rMeridian*(alpha*this.p.PI/180), 0, -this.rMeridian*(alpha*this.p.PI/180), 1-this.vertexY);
      this.p.line(this.rMeridian*(alpha*this.p.PI/180), 0, this.rMeridian*(alpha*this.p.PI/180), 1-this.vertexY);
    }
    ...
    this.p.stroke(0,0,255);
    if (this.drawingMode != "log") {
      this.p.line(-1, this.rMeridian, 1, this.rMeridian);
    }
    let rotatedDisks = [];
    ...
    //draw disks
    for(let disk of this.extraDisks) {
      disk.displayDiskCylinder([240, 240, 240, 230], [200,200,200,230], this.angle, this.rMeridian,this.drawingMode);
    }
    
    for (let disk of this.disks) {
      disk.displayDiskCylinder(180, [0,0,0,0], this.angle, this.rMeridian, this.drawingMode);
  ...
  drawAxes() {
    ...
    this.p.strokeWeight(3/this.windowHeight);

    //draw lines at 30° intervals from cone vertex

    let axesLength = this.p.sqrt((this.windowWidth/this.windowHeight + this.p.abs(this.vertexX))**2+(1+this.p.abs(this.vertexY))**2);
  ...
  drawAxesCylinder() {
    ...
    for(let angle = 0; angle <= 180; angle += 15) {
      if (this.drawingMode == "log") {
        this.p.line(-(angle-90)/this.angle,0,-(angle-90)/this.angle, 1-this.vertexY)
      }
      else {
        this.p.line(-this.rMeridian*((angle-90)*this.p.PI/180),0,-this.rMeridian*((angle-90)*this.p.PI/180), 1-this.vertexY)
      }
    }

    //draw horizontals
    let conicAxesLength = this.p.sqrt((this.windowWidth/this.windowHeight + this.p.abs(this.vertexX))**2+(1+this.p.abs(this.vertexY))**2);
    ...
    for(let r = 0; r < axesLength; r += circleInterval) {
      if (this.drawingMode == "log") {
        ...
        this.p.line(-axesLength, h, axesLength, h)
        this.p.pop();
      } else {
        this.p.line(-axesLength, r, axesLength, r)
      }
    }
  ...
  createTransform() {
    ...
    this.p.scale(this.windowHeight/2);
    this.p.scale(1,-1);
  ...
  createTransformCylinder() {
    this.p.translate(3*this.windowWidth/2, this.windowHeight/2);
    //before, we're assuming 1 = 100%
    this.p.scale(this.windowHeight/2);
    this.p.scale(1,-1);

    this.p.translate(this.vertexX, this.vertexY);
    //this.p.translate(0,1/2*this.windowHeight)
  }

  
  mainLinesSVGCylinder() {
    let linesStr = '<g stroke="black" stroke-width="' + 5/this.windowHeight +'">\n' 
    lineStr += lineSVG(-this.rMeridian*(this.angle*this.p.PI/180), 0, -this.rMeridian*(this.angle*this.p.PI/180), 1-this.vertexY);
    lineStr += lineSVG(this.rMeridian*(this.angle*this.p.PI/180), 0, this.rMeridian*(this.angle*this.p.PI/180), 1-this.vertexY)
    lineStr += '</g>\n'
    
    let meridStr = '<g stroke="blue" stroke-width="' + 4/this.windowHeight +'">\n' 
    meridStr += lineSVG(-1, this.rMeridian, 1, this.rMeridian)
    meridStr += '</g>\n'
    
    return linesStr + meridStr;
  }
  gridLinesSVGCylinder() {
    let returnStr = '<g stroke="rgb(255 175 175)" fill="rgb(255 175 175 / 0)" stroke-width="'+ 3/this.windowHeight + '">\n'
    ...
    this.p.angleMode(this.p.DEGREES);
    for(let angle = 0; angle <= 180; angle += 15) {
      returnStr += lineSVG(-this.rMeridian*((angle-90)*this.p.PI/180),0,-this.rMeridian*((angle-90)*this.p.PI/180), 1-this.vertexY)
    ...
    for(let r = 0; r < axesLength; r += circleInterval) {
      returnStr += lineSVG(-axesLength, r, axesLength, r)
    }
    returnStr += '</g>\n'
  }
  disksSVGCylinder() {
  ...
  frontsSVGCylinder() {
    let returnStr = '<g stroke-width="'+ 5/this.windowHeight + '">\n';
    ...
    for(let index = 0; index < this.front.length - 1; index ++) {
      ...
      if(this.isUpSegment(disk1, disk2)) {
        // this.p.stroke(200, 0, 0);
        returnStr += '<g stroke="rgb(200 0 0)">\n'
      }
      else {
        // this.p.stroke(200, 255, 0);
        returnStr += '<g stroke="rgb(200 255 0)">\n'
      }
      
      returnStr += lineSVG(disk1.cylinderX, disk1.cylinderY, disk2.cylinderX, disk2.cylinderY);
      returnStr += '</g>\n'
    }
    ...
    if(this.areTouching(rotatedFirstDisk, lastDisk)) {
      ...
      returnStr += lineSVG(rotatedFirstDisk.cylinderX, rotatedFirstDisk.cylinderY, lastDisk.cylinderX, lastDisk.cylinderY);
      returnStr += '</g>\n'
    }
    returnStr += '</g>\n'
    return returnStr
  }
  ontologicalGraphSVGCylinder() {
    let returnStr = '<g stroke="rgb(0 0 200)" stroke-width="'+ 3/this.windowHeight + '">\n'

    // this.p.stroke(0,0,200);
    // this.p.strokeWeight(3/this.windowHeight);
    for (let disk of this.disks) {
      if(disk.parents.length >= 1) {
        returnStr += lineSVG(disk.parents[0].cylinderX, disk.parents[0].cylinderY, disk.cylinderX, disk.cylinderY);
      }
      if(disk.parents.length >= 2) {
        returnStr += lineSVG(disk.parents[1].cylinderX, disk.parents[1].cylinderY, disk.cylinderX, disk.cylinderY);
        }
      }
    returnStr += '</g>\n'
    return returnStr
  ...
  gridLinesSVGCylinder() {}
  disksSVG() {
    let mainDisksSVG = genDisksSVG(this.disks, 180, [0,0,0,0])
    let extraDisksSVG = genDisksSVG(this.extraDisks, [240, 240, 240, 230], [200,200,200,230])
    return extraDisksSVG + mainDisksSVG
  }
  frontsSVGCylinder() {}
  createTransformStr() {
    return '"translate('+this.windowWidth/2 + ' ' + this.windowHeight/2+')\n scale('+this.windowHeight/2+') \n scale(1 -1)\n translate('+ this.vertexX + ' ' + this.vertexY + ')"'
  }
  createTransformStrCylinder() {
    return '"translate('+this.windowWidth/2 + ' ' + 3*this.windowHeight/2+')\n scale('+this.windowHeight/2+') \n scale(1 -1)\n translate('+ this.vertexX + ' ' + this.vertexY + ')"'
  }
  applyTransformSVG(transformStr, svgStr) {
    let returnStr = '<g transform=' + transformStr + '>\n'
    returnStr += svgStr
    returnStr += '</g>\n'
    return returnStr
  }
