...
/********************************************
SURF 2023
Copyright (c) 2023 Elaine Demetrion and Emi Neuwalder
Licensed under the MIT license: http://www.opensource.org/licenses/mit-license.php 
******************************************* */
...
class Disk {
  ...
  constructor(p, x, y, radius, id=-1, alpha=25, rMeridian=0.8) {
    ...
    this.x = x;
    this.y = y;
    this.dist2 = x**2 + y**2 // distance from the origin squared
    this.radius = radius;
  ...
  displayDiskCylinder(fillColor = [200], strokeColor = [0], alpha, rMeridian, mode, p) {
    ...
    else if (mode == "fixedCircle") {
      p.fill(fillColor);
      p.stroke(strokeColor);
      p.strokeWeight(this.radius*0.1);
      p.ellipse(this.cylinderX, this.cylinderY, 2*this.radius, 2*this.radius);
    }
    else {
...
/** Additional functions to help with cylindrical visualization displays: */
...
function logarithmicCylinderFromCone(x,y,alpha,p) {
  ...
  let argZ = p.atan2(y,x)
  if (x == 0 && y == 0) {
    throw "In calculating logarithmic transformation, encountered disk with x=y=0."
  }
